...
export function decideField(label: string, type: string, context: ApplyContext): FieldDecision {
  ...
  if (/phone|mobile/.test(key)) {
    return { action: "pause", category: "contact", reason: "Phone number is not verified in the current Odesseus profile." };
  }

  return { action: "pause", category: "custom", reason: "Odesseus does not have a pre-approved answer for this question." };
}
