  ...
  try {
      ...
      if (/^(yes|true|1)$/i.test(decision.value)) await locator.check();
        ...
        return {
          ...
          pause: {
            ...
            category: "custom",
            reason: "Odesseus needs the user to confirm this checkbox.",
          },
          source: null,
        };
      }
    ...
    return {
      ...
      pause: {
        ...
        question: cleanLabel(field.label || field.name || field.placeholder || "Application question"),
        category: "custom",
        reason: "Odesseus could not confidently complete this field.",
      },
      source: null,
    };
  }
...
export async function runApplicationPass(runId: string, command: ApplyCommand) {
  ...
  try {
    ...
    if (!actionButton) {
      const reason = "Odesseus could not identify the next or final application control. Review the page in the live browser.";
      await supabase
        .from("application_runs")
        .update({
          status: "needs_user",
