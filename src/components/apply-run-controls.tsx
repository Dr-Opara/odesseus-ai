...
export default function ApplyRunControls({
  ...
  async function command(action: "continue" | "submit" | "cancel") {
    ...
    if (!response.ok) {
      setError(data.error || "Odesseus could not continue.");
      setBusy(false);
      return;
    }
  ...
  async function resolve() {
    ...
    if (!response.ok) {
      setError(data.error || "Odesseus could not save your answers.");
      setBusy(false);
      return;
    }
    ...
    return (
      <div className="card apply-question-card">
        <div>
          ...
          <h2 style={{ margin: "7px 0 6px" }}>Odesseus needs a few answers.</h2>
  ...
  if (!["submitted", "failed", "cancelled"].includes(status)) {
    return <span className="muted">Odesseus is working…</span>;
  }
