...
export default function ApplyStartForm({ jobId, defaultUrl, mode }: {
  jobId: string;
  defaultUrl?: string | null;
  mode: "apply" | "smart_apply";
}) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      if (data.runId) { router.push(`/apply/run/${data.runId}`); return; }
  ...
  return (
    <form onSubmit={submit} className="apply-start-card">
      <label className="field-label">Application page
        <input className="input" type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://company.com/jobs/..." required />
      </label>
