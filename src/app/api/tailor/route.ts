...
export async function POST(request: Request) {
  ...
  if (!job?.description) {
    return NextResponse.json({ error: "Odesseus could not find this job description." }, { status: 404 });
  }
  ...
  try {
    ...
    if (!parsed.success) {
      ...
      if (downloadError || !file) throw new Error("Odesseus could not read your resume.");
      ...
      if (cacheError) throw new Error("Odesseus could not save the parsed resume.");
    }
    ...
    if (insertError || !tailoring) {
      throw new Error("Odesseus could not save the tailored resume.");
    }
