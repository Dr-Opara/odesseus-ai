# Developing Odesseus with Codex in VS Code

Odesseus v0.1-v0.11 has been feature-built. The next Codex phase is **integration and stabilization**, not greenfield feature development.
...
```bash
git checkout build/odesseus-post-interview-v0.11
git pull
git checkout -b stabilize/odesseus-v0.11
```
...
> Read AGENTS.md, docs/development/integration-audit.md, README.md, package.json, and the current branch before editing anything. Odesseus v0.1-v0.11 is already feature-built. Your job is to stabilize it for a real local build and Vercel Preview deployment without redesigning the product. Start with P0 items only. Install dependencies and commit a lockfile, run lint/typecheck/build, fix compile errors, baseline the current Supabase remote schema into reproducible migrations, generate Supabase TypeScript types, add tests/CI, fix the documented Odesseus Live state/WebRTC/order/graceful-close issues, and make Apply successful-submission plus credit accounting atomic. Preserve all product/security rules in AGENTS.md. Report every change, command run, test result, and remaining blocker.
...
- Make stabilization changes on `stabilize/odesseus-v0.11`.
